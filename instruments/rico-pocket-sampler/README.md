...
- **Per-pad flavor** — select a pad, then drag **PITCH** (−12…+12 semitones) to
  transpose it (chipmunk / baritone), hit **Reverse** to play it backwards, and
  balance the kit with per-pad **VOL** (0–200%) and **PAN** (L/R) controls. Gain
  and pan are applied per hit and baked into the WAV export too.
...
- **Pattern bank** — four save slots (`Pattern` row). Hit **Save** to stash the
  ...
  **Clear** to erase the selected slot. Patterns persist across reloads via
  `localStorage`.
- **Persistence** — your whole kit (all 8 pads: sample buffers plus their
  pitch, trim, reverse, volume, pan, mute and solo state) is autosaved to
  **IndexedDB** and restored on reload, so loaded / recorded samples survive a
  refresh. Buffers are stored as raw interleaved `Float32` ArrayBuffers.
- **Export WAV** — render the whole mixdown to a downloadable `.wav`. An
  `OfflineAudioContext` rebuilds the FX chain (drive / tone / delay / space /
  limiter), schedules every active step across 4 bars (pitch, reverse, choke,
  mute and solo all respected), and bakes the loop into a stereo PCM file.
