...
class ScratchProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs) {
    ...
    const frames = numCh ? out[0].length : 0;
    const len = this.buf.length;
    if (!len) return true;
    // Advance the playhead/rate ONCE per sample frame, then fan the same
    // mono sample out to every output channel. Advancing per-channel (the
    // old bug) doubled the effective playback rate and left channels
    // holding time-offset copies of each other -- harmless-ish on true
    // stereo output, but a mono-summed device (most phone speakers) would
    // comb-filter/cancel the two copies into distorted mush.
    for (let i = 0; i < frames; i++) {
      ...
      const frac = ph - i0;
      const sample = this.buf[i0] * (1 - frac) + this.buf[i1] * frac;
      for (let ch = 0; ch < numCh; ch++) out[ch][i] = sample;
    }
    return true;
  }
}
...
`;
  ...
  // Wrap a raw mono Float32Array in a real AudioBuffer, for use with native
  // AudioBufferSourceNode (unlike the scratch engine, which reads Float32Array
  // samples directly, AudioBufferSourceNode.buffer requires an AudioBuffer).
  function toAudioBuffer(ctx, floatArray) {
    const buf = ctx.createBuffer(1, floatArray.length, ctx.sampleRate);
    buf.copyToChannel(floatArray, 0);
    return buf;
  }
  ...
  function ensureAudio() {
    ...
    state.crackleBuffer = toAudioBuffer(ctx, makeCrackle(ctx.sampleRate, 2.0));
    ...
    crackleSrc.start();
    state.needleBuffer = toAudioBuffer(ctx, makeNeedle(ctx.sampleRate));
    applyAmbient();
