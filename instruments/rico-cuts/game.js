...
(function () {
  ...
  // =======================================================================
  // `amount` is 0..1 (straight from the Drive slider). At 0 the curve is
  // the identity line (no coloration at all); as amount rises it eases into
  // tanh saturation, normalized so a full-scale input (x = ±1) always maps
  // to output ±1 instead of being under-driven.
  ...
  function buildFX() {
    ...
    state.shaper.oversample = "4x"; // smooths out the harsh aliasing waveshaping otherwise adds
    // curve is set from the live Drive slider in applyFx() below, not fixed here
  ...
  function ensureAudio() {
    ...
    // Limiter: the dry scratch signal and the echo tail (which can build
    // up via its own feedback loop) both land on gainNode independently,
    // and the Drive waveshaper's saturation ceiling doesn't cover that
    // combined sum. Pushing Drive/Echo/Volume together previously had no
    // ceiling and could clip hard at the destination. Same fix already
    // applied in Rico's EQ for the identical reason.
