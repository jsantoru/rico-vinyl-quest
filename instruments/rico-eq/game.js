...
(function () {
  ...
  function initAudio() {
    ...
    // Chain: target[0..4] -> user[0..4] -> analyser -> master -> limiter -> out
    // The limiter is a safety net, not tone shaping: five bands stacked in
    // series can each add up to +15dB, so pushing a few bands hard is easy
    // to do by ear and previously meant hard digital clipping at the
    // destination -- audibly harsh in a way that doesn't teach anything
    // about EQ. A fast limiter lets big boosts still sound loud/aggressive
    // without ever wrapping/clipping.
