...
(() => {
  ...
  style.textContent = `
    ...
    #dpadUp {
      left: 68px; bottom: 156px;
      width: 56px; height: 56px; font-size: 18px;
    }

    #dpadDown {
      left: 68px; bottom: 88px;
      width: 56px; height: 56px; font-size: 18px;
    }

    #dpadLeft {
      left: 18px; bottom: 122px;
      width: 56px; height: 56px; font-size: 18px;
    }

    #dpadRight {
      left: 118px; bottom: 122px;
      width: 56px; height: 56px; font-size: 18px;
    }

    #btnE {
      right: 22px; bottom: 116px;
      width: 78px; height: 78px;
      border-radius: 50%; font-size: 20px;
    }

    #btnB {
      right: 114px; bottom: 148px;
      width: 54px; height: 54px;
      border-radius: 50%;
    }

    #btnM {
      right: 114px; bottom: 78px;
      width: 54px; height: 54px;
      border-radius: 50%;
    }
  `;
  ...
  function fitCanvas() {
    ...
    canvas.style.width =
      Math.max(1, Math.floor(VIEW_W * scale)) + 'px';

    canvas.style.height =
      Math.max(1, Math.floor(VIEW_H * scale)) + 'px';
  }
...
const RECORDS = {
  elm: {
    title: 'Elm Street Funk',
    artist: 'Static Groove',
    year: '1974',
    sample: 'Drum Break',
    layer: 'drums',
    color: '#e0a030',
  ...
  cola: {
    title: 'Cherry Cola Bounce',
    artist: 'Rosie & The Fizz',
    year: '1968',
    sample: 'Bassline',
    layer: 'bass',
    color: '#d04830',
...
function makeOverworld() {
  ...
  map.crates[key(26,20)] = {
    junkSeed: 3
  };

  map.crates[key(28,21)] = {
    record: 'white'
  };

  map.crates[key(30,20)] = {
    junkSeed: 6
  };
...
const shops = {
  groove: makeShop('groove', {
    ...
    keeper: {
      ...
      lines: [
        'Rico! My man. Diggers been through all week and found nothing.',
      ...
      foundLine:
        'You FOUND it?! Elm Street Funk, in my own shop. Flip that break into something beautiful.'
    },
  ...
  wax: makeShop('wax', {
    ...
    keeper: {
      ...
      lines: [
        'Welcome to the Wax Museum. Look, don’t touch. Unless you’re buying. Then touch.',
      ...
      foundLine:
        'Midnight Stab, in the flesh. Those horns are going to slap so hard.'
    },
  ...
  diner: makeShop('diner', {
    ...
    keeper: {
      ...
      lines: [
        'Sit anywhere, hon. Kitchen’s slow but the jukebox is fast.',
      ...
      foundLine:
        'Well I’ll be. Make that bassline bounce again, sugar.'
    },
  ...
  thrift: makeShop('thrift', {
    ...
    keeper: {
      ...
      lines: [
        'Everything’s a dollar. Except the stuff that isn’t.',
      ...
      foundLine:
        'Galactic Hallelujah! I almost priced that at fifty cents. Glad it’s you.'
    },
...
for (const [id, d] of Object.entries(doors)) {
  transitions['town:' + key(d.doorX, d.doorY)] = {
    map: id,
    x: 6.5,
    y: 7.5
  };

  transitions[id + ':' + key(6, 9)] = {
    map: 'town',
    x: d.doorX + 0.5,
    y: d.doorY + 1.6
  };
...
function toggleSkate() {
  if (
    state !== 'play' ||
    !maps[player.map].outside
  ) return;

  player.skating = !player.skating;

  toast = {
    text: player.skating ? 'Skateboard: ON' : 'Skateboard: OFF',
    t: 1.2
  };
}
...
const music = {
  ...
  start() {
    ...
    this.noiseBuf = this.ctx.createBuffer(
      1,
      len,
      this.ctx.sampleRate
    );
  ...
  toggleMute() {
    ...
    toast = {
      text: this.muted ? 'Music: MUTED' : 'Music: ON',
      t: 1.2
    };
  },
...
function doInteract() {
  ...
  if (target.type === 'keeper') {
    ...
    if (
      c.record &&
      !collected.has(c.record)
    ) {
      collected.add(c.record);

      music.enable(
        RECORDS[c.record].layer
      );
      ...
      dialog = {
        name: 'CRATE',
        lines: [
          'Nothing left in here but dust and old sleeves.'
        ],
        i: 0
      };
...
function drawBuildings(map) {
  for (const b of map.buildings) {
    ...
    // Green Door Studio gets a darker brick look.
    ...
    // ------------------------------------------------------------
    // HEY BUD TREE / PLANT DECORATIONS
    // ------------------------------------------------------------
...
function drawHeyBudDecor(
  ...
  // little potted plants near the building
  drawPlantPot(
    px + 8,
    py + h - 5
  );
...
function drawTownDecorations(time) {
  // Green Door Studio art setup
  drawGreenDoorArtArea();

  // Kountry Kart Deli sidewalk scene
  drawDeliScene(time);

  // Pure Pop Records coffee cart
  drawCoffeeCart();
...
// ---------------------------------------------------------------- Green Door Studio art area
function drawGreenDoorArtArea() {
  const baseX = 3 * TILE;
  const baseY = 7 * TILE + 4;

  // Small folding art table
  ctx.fillStyle =
    '#513628';
